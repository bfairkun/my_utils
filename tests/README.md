...
The tests compute a checksum of the first 10MB of the FASTA file for validation. On first run, the checksum will be printed. You can add it to `EXPECTED_FASTA_SHA256` in the test file for future validation.

If tests are skipped due to missing FASTA, you'll see helpful error messages with download instructions.
...
- Tests use pytest fixtures to load SpliceAI models once per session for efficiency
