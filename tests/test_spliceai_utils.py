...
def test_variant_effect_comparison(fasta_path, spliceai_models, test_variant):
    ...
    print("Donor probability at cryptic site:")
...
def test_spliceO_predictions_with_immutable_range(fasta_path, spliceai_models):
    ...
    print(
        f"✓ Predictions with immutable range completed: {len(results_immutable)} rows"
    )
    ...
    if len(masked_rows) > 0:
        ...
        print(
            "  Note: Immutable bases should be preserved (not N's) when they overlap the mask"
        )
